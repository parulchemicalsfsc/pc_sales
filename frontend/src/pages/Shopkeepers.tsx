...
export default function Shopkeepers() {
  ...
  const [editingShopkeeper, setEditingShopkeeper] =
    useState<Shopkeeper | null>(null);
  const [searchTerm, setSearchTerm] = useState("");
  const [columnNames, setColumnNames] = useState<Record<string, string>>(() => {
    const saved = localStorage.getItem("shopkeeperColumnNames");
    return saved ? JSON.parse(saved) : {};
  });

  useEffect(() => {
    localStorage.setItem("shopkeeperColumnNames", JSON.stringify(columnNames));
  }, [columnNames]);
  ...
  const handleDeleteClick = (row: Shopkeeper) => {
    setSelectedShopkeeper(row);
    setDeleteOpen(true);
  };

  const handleConfirmDelete = async () => {
    if (!selectedShopkeeper?.shopkeeper_id) return;
    console.log("[DELETE] Module: Shopkeeper | Record ID:", selectedShopkeeper.shopkeeper_id);
    try {
      await shopkeeperAPI.delete(selectedShopkeeper.shopkeeper_id);
      console.log("[DELETE SUCCESS] Module: Shopkeeper | Record ID:", selectedShopkeeper.shopkeeper_id);
      setToast({ open: true, message: "Shopkeeper deleted successfully", severity: "success" });
      loadShopkeepers();
    } catch (error: any) {
      console.error("[DELETE FAILED] Module: Shopkeeper | Error:", error);
      setToast({ open: true, message: error?.message || "Failed to delete shopkeeper", severity: "error" });
    } finally {
      setDeleteOpen(false);
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : t("shopkeepers.loadError", "Failed to load Mantri"),
      );
  ...
  const handleOpenDialog = (shopkeeper?: Shopkeeper) => {
    if (shopkeeper) {
      ...
      setFormData({
        ...
        state: toUpperCaseText(shopkeeper.state),
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (err?.isNetworkError || err?.response?.status >= 500) {
        setError("Network error or server error. Please check if shopkeeper was saved before trying again.");
      } else {
        setError(
          err instanceof Error
            ? err.message
            : t("shopkeepers.saveError", "Failed to save Mantri"),
        );
      }
  ...
  const getRowColor = (row: Shopkeeper) => {
    ...
    const isRed = redFields.some(
      (field) =>
        ...
        row[field as keyof Shopkeeper] === undefined ||
        row[field as keyof Shopkeeper] === "",
    );
    if (isRed) return "red";
  ...
  const baseColumns: GridColDef[] = [
    ...
    {
      field: "record_date",
      headerName: t("shopkeepers.recordDate", "Record Date"),
      width: 150,
      minWidth: 120,
      headerAlign: "center",
      align: "center",
  ...
  const filteredShopkeepers = shopkeepers.filter((shopkeeper) =>
    Object.values(shopkeeper).some((value) =>
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          ...
          >
            ...
            />
            <PermissionGate permission={PERMISSIONS.CREATE_SHOPKEEPER}>
              <Button
                variant="contained"
                startIcon={<AddIcon />}
                onClick={() => handleOpenDialog()}
              >
                {t("shopkeepers.addShopkeeper")}
              </Button>
            </PermissionGate>
            <IconButton onClick={loadShopkeepers} color="primary">
              <RefreshIcon />
            </IconButton>
          </Box>
        </CardContent>
        ...
        <CardContent>
          <Box sx={{ height: 600, width: "100%", overflowX: "auto" }}>
            {loading ? (
              ...
              <DataGrid
                rows={filteredShopkeepers}
                columns={columns}
                getRowId={(row) => row.shopkeeper_id}
                columnBuffer={10}
          ...
          <Grid container spacing={2} sx={{ mt: 1 }}>
            ...
            <Grid item xs={12}>
              <Typography variant="subtitle2" color="primary" sx={{ fontWeight: 600 }}>
                {t("shopkeepers.basicInfo", "Basic Information")}
              </Typography>
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                fullWidth
                type="date"
                label={t("shopkeepers.recordDate", "Record Date")}
            ...
            <Grid item xs={12} sx={{ mt: 2 }}>
              <Typography variant="subtitle2" color="primary" sx={{ fontWeight: 600 }}>
                {t("shopkeepers.sabhasadDetails", "Sabhasad & Participation")}
              </Typography>
              <Divider sx={{ mt: 0.5, mb: 1.5 }} />
            </Grid>
            <Grid item xs={12} sm={4}>
              <TextField
                fullWidth
                type="number"
                label={t("shopkeepers.sabhasadMorning", "Sabhasad Morning")}
            ...
            <Grid item xs={12} sm={4}>
              <TextField
                fullWidth
                type="number"
                label={t("shopkeepers.sabhasadEvening", "Sabhasad Evening")}
            ...
            <Grid item xs={12} sm={4}>
              <TextField
                fullWidth
                type="number"
                label={t("shopkeepers.sabhasadCount", "Total Sabhasad")}
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                fullWidth
                type="number"
                label={t("shopkeepers.contactInGroup", "Contact in Group")}
            ...
            <Grid item xs={12} sx={{ mt: 2 }}>
              <Typography variant="subtitle2" color="primary" sx={{ fontWeight: 600 }}>
                {t("shopkeepers.dairyOps", "Dairy Operations")}
              </Typography>
              <Divider sx={{ mt: 0.5, mb: 1.5 }} />
            </Grid>
            <Grid item xs={12} sm={6}>
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                fullWidth
                select
                label={t("shopkeepers.nature", "Nature of Sabhasad")}
            ...
            <Grid item xs={12} sm={3}>
              <TextField
                fullWidth
                type="time"
                label={t("shopkeepers.dairyTimeM", "Dairy Time (M)")}
            ...
            <Grid item xs={12} sm={3}>
              <TextField
                fullWidth
                type="time"
                label={t("shopkeepers.dairyTimeE", "Dairy Time (E)")}
            ...
            <Grid item xs={12} sm={3}>
              <TextField
                fullWidth
                type="number"
                label={t("shopkeepers.milkM", "Milk (M)")}
            ...
            <Grid item xs={12} sm={3}>
              <TextField
                fullWidth
                type="number"
                label={t("shopkeepers.milkE", "Milk (E)")}
            ...
            <Grid item xs={12} sx={{ mt: 2 }}>
              <Typography variant="subtitle2" color="primary" sx={{ fontWeight: 600 }}>
                {t("shopkeepers.businessInsights", "Business & Recovery")}
              </Typography>
              <Divider sx={{ mt: 0.5, mb: 1.5 }} />
            </Grid>
            <Grid item xs={12} sm={6}>
              <TextField
                fullWidth
                type="number"
                label={t("shopkeepers.recoveryDemo", "Recovery Days (Demo)")}
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                fullWidth
                type="number"
                label={t("shopkeepers.recoveryDispatch", "Recovery Days (Dispatch)")}
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                fullWidth
                select
                label={t("shopkeepers.dmAvailM", "DM Avail (M)")}
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                fullWidth
                select
                label={t("shopkeepers.dmAvailE", "DM Avail (E)")}
            ...
            <Grid item xs={12}>
              <TextField
                fullWidth
                select
                label={t("shopkeepers.support", "Support Level")}
            ...
            <Grid item xs={12}>
              <TextField
                fullWidth
                select
                label={t("shopkeepers.highLowVillages", "High to Low Holder Villages")}
            ...
            <Grid item xs={12}>
              <TextField
                fullWidth
                select
                label={t("shopkeepers.businessStatus", "Current Status of Business")}
